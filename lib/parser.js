...
var cdlre = (function (cdlre) {
    ...
    var Assertion = (function() {
        var kinds = new Set([
            'BeginningOfLine', 'EndOfLine', 'WordBoundary', 'NotWordBoundary',
    ...
    PatternCharacter.BAD = new Set([
        '^', '$', '.', '*', '+', '?', '(', ')', '[', ']', '{', '}', '|', BACKSLASH
    ]);

    var Quantifier = (function() {
        function QuantifierPrefix(kind, value) {
            var kinds = new Set(['Star', 'Plus', 'Question', 'Fixed', 'LowerBound', 'Range']);
            if (!kinds.has(kind))
    ...
    var AtomEscape = (function() {
        function CharacterClassEscape(kind) {
            var kinds = new Set(['Word', 'NotWord', 'Digit', 'NotDigit', 'Space', 'NotSpace']);
            if (!kinds.has(kind))
        ...
        function CharacterEscape(kind, value) {
            var kinds = new Set([
                'ControlEscape', 'ControlLetter', 'HexEscapeSequence',
        ...
        return {
            ...
            CharacterEscape: {
                ...
                ControlEscape: {
                    t: AtomEscape(undefined, CharacterEscape('ControlEscape', '\u0009'), undefined),
                    n: AtomEscape(undefined, CharacterEscape('ControlEscape', '\u000A'), undefined),
                    v: AtomEscape(undefined, CharacterEscape('ControlEscape', '\u000B'), undefined),
                    f: AtomEscape(undefined, CharacterEscape('ControlEscape', '\u000C'), undefined),
                    r: AtomEscape(undefined, CharacterEscape('ControlEscape', '\u000D'), undefined),
                },
    ...
    var ClassAtomNoDash = (function() {
        var kinds = new Set(['ClassEscape', 'SourceCharacter']);
    ...
    var NonemptyClassRangesNoDash = (function() {
        var kinds = new Set(['ClassAtom', 'Dashed', 'NotDashed']);
    ...
    var Atom = (function() {
        var kinds = new Set([
            'PatternCharacter', 'Dot', 'AtomEscape', 'CharacterClass',
        ...
        function Atom(kind, value) {
            ...
            return {
                ...
                value: value,
                toString: function() {
                    if (new Set(['CapturingGroup', 'PatternCharacter']).has(this.kind))
                        return this.nodeType + '.' + this.kind + '(' + this.value + ')';
                    if (this.kind === 'Dot')
    ...
    function parseClassAtomNoDash(scanner) {
        ...
        if (new Set([BACKSLASH, ']', '-']).has(scanner.next()))
     ...
     */
    parseAlternative.BAD_START = cdlre.SetDifference(
